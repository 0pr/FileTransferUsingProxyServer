#include <sys/socket.h>  
 ...
 {  
   ...
   int bytes =0;  
      ...
      recv(info->client_fd, b1, sizeof(b1), 0);
      bzero(buffer, 512);
      printf("Searching for %s in proxy server directory.\n", b1);
      f = fopen(b1, "r") ;
      if(f==NULL){
	printf("File not found. Connecting to server\n") ;
	send(server_fd, b1, sizeof(b1), 0);
	FILE *fp ;
	int ch = 0 ;
	fp = fopen(b1, "a") ;
        int words ;
        read(server_fd, &words, sizeof(int)) ;
        while(ch!=words){
             recv(server_fd, buffer, 512, 0);
	...
	f = fopen(b1, "r") ;
	printf("Transmitting to client.\n") ;
      	while((c=getc(f))!=EOF){
		fscanf(f, "%s", buffer);
		if(isspace(c)||c == '\t' || c == '\n') words++ ;
	}
	write(info->client_fd, &words, sizeof(int));
	rewind(f);
	char ch ;
	while(ch!=EOF){
		fscanf(f, "%s", buffer);
		send(info->client_fd, buffer, 512, 0);
		ch = fgetc(f);
	}
   return NULL;  
 }  
